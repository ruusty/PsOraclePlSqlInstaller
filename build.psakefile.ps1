...
properties {
  ...
  $GlobalPropertiesPath = Ruusty.ReleaseUtilities\Find-FileUp "GisOms.Chocolatey.properties.${env:COMPUTERNAME}.xml" -verbose:$IsVerbose
  ...
  $exFile = @("*.nuspec", ".gitignore", "*.config.ps1", "*.lis", "*.nupkg", "*.Tests.ps1", "*.html", "*Pester*", "*.Tests.Setup.ps1")
  #Quote the elements
  $XD = ($exDir | %{ "`"$_`"" }) -join " "
  $XF = ($exFile | %{ "`"$_`"" }) -join " "
...
task Compile -description "Build Deliverable zip file" -depends clean, create-dirs, set-version   {
  ...
  Get-ChildItem -Recurse -Path $ProjBuildPath -Filter "*.psd1" | %{
    Ruusty.ReleaseUtilities\Set-VersionModule -path $_.FullName  -version $version -Verbose:$IsVerbose
  }
...
#  GisOmsUtils\Get-GitFile -gitRemote $(join-path -path $SpatialGitHubPath -child "ChocoPkgContents/PSGisOmsRelease.git" ) -gitBranch "master" -gitFilePath "GisOmsRelease\Public\Get-GisOmsSdlc.ps1" -destPath $ProjBuildPath -verbose:$IsVerbose
